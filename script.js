...
const contact_me_btn = document.getElementById("contact_me_btn");
...
contact_me_btn.addEventListener('click', e => {
    e.preventDefault();
    contact_me_alert();
});
...
const mag_validation = (alias) => {
    ...
    if(typeof(vector_mag) === 'number' && vector_mag <= 99999999999999 && vector_mag >= 0){
...
const proceed_calculation = () => {
    if (pockets_occupied == 2) {
        ...
        show_vector_result(Resultant, Theta, Rcost, Rsint);
...
const show_vector_result = (resultant, theta, rcost, rsint) => {
    ...
    if (rcost >= 0 && rsint >= 0) {
        if (theta >= 0 || theta <= 0) {
            ...
            resulatant_span_view.innerHTML = 0;
            direction_span_view.innerHTML = 0;

            visual_simulation_btn.style.pointerEvents = 'auto';
            visual_simulation_btn.style.display = 'flex';

            vct_results.resultant = 'abnormal';
            vct_results.direction = 'abnormal';
        }
    }else if (rcost < 0 && rsint > 0) {
        const prop_theta = 180 - Math.abs(theta.toFixed(3));
        resulatant_span_view.innerHTML = resultant.toFixed(3);
        direction_span_view.innerHTML = prop_theta;

        visual_simulation_btn.style.pointerEvents = 'auto';
        visual_simulation_btn.style.display = 'flex';

        vct_results.resultant = resultant;
        vct_results.direction = prop_theta;
    }else if (rcost < 0 && rsint < 0) {
        const prop_theta = 180 + Math.abs(theta.toFixed(3));
        resulatant_span_view.innerHTML = resultant.toFixed(3);
        direction_span_view.innerHTML = prop_theta;
        ...
        const prop_theta = 360 - Math.abs(theta.toFixed(3));
        resulatant_span_view.innerHTML = resultant.toFixed(3);
        direction_span_view.innerHTML = prop_theta;
...
const show_error = (err_code) => {
    if (err_code == 99) {
        ...
        Swal.fire({
          title: 'Number Error!',
          text: 'Please use number within 0 to 99999999999999.',
          icon: 'error',
          confirmButtonText: 'Okay!'
        })
    }
...
async function contact_me_alert() {
    ...
    const { value: selection } = await Swal.fire({
      title: "Select a platform to contact me",
      input: "radio",
      inputOptions,
